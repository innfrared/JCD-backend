...
class DjangoProductCardRepository(ProductCardRepository):
    ...
    def _subcategory_ids_from_model(product_model: ProductModel) -> List[int]:
        """Read subcategory IDs without per-product queries."""
    ...
    def get_product_cards(
        ...
        # Fetch products with deterministic primary image in one batched query.
        product_models = with_resolved_primary_image(ProductModel.objects.filter(
            id__in=product_ids
        ...
        for pid in product_ids:
            if pid in product_map:
                ...
                product._image_url = product.variant_image  # type: ignore
